#!/bin/bash
ARG_DESIGNSOURCE=../verilog/s9234_comb5new.v
ARG_WORKDIR=comb_dir
TEMP_FILE=temp
LOG_DIR=$ARG_WORKDIR/logs5new
PFAULT_DIR=pfaults
...
echo "report_faults faulttype=static reportcircuit=yes faultstatus=untested experiment=exp_comb testmode=FULLSCAN inputcommitted=no workdir=$ARG_WORKDIR > $LOG_DIR/report_faults.out" >> $TEMP_FILE
