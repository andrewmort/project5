...
echo "build_faultmodel includeignore=no includedynamic=no  precollapsed=no workdir=$ARG_WORKDIR ignoreblackbox=yes cellfaults=no > $LOG_DIR/3_build_faultmodel.out" >> $TEMP_FILE  
...
echo "simulate_vectors WORKDIR=$ARG_WORKDIR TESTMODE=FULLSCAN EXPERIMENT=exp_scan faulttype=static simulation=gp importfile=$INPUTVECTOR > $LOG_DIR/5_simulate_vectors.out" >> $TEMP_FILE
