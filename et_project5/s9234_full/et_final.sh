...
echo "verify_test_structures WORKDIR=$ARG_WORKDIR TESTMODE=FULLSCAN > $LOG_DIR/3_verfiy_test_structures.out" >> $TEMP_FILE

echo "echo Simulating Vectors" >> $TEMP_FILE
echo "simulate_vectors WORKDIR=$ARG_WORKDIR TESTMODE=FULLSCAN EXPERIMENT=exp_scan faulttype=static importfile=$INPUTVECTOR > $LOG_DIR/4_simulate_vectors.out" >> $TEMP_FILE
